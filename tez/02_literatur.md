...
Tarama PRISMA 2020 kılavuzu [9] esas alınarak
...
[1]. Wang, frekans-zaman düzleminde tepe noktalarını
...
genel bakış makalesi [2], akustik parmak izi yöntemlerini
...
projesi [3] chroma vektörlerine dayalı bir alternatif
...
[4] ise zaman damgalı onset (vuruş başlangıcı) tabanlı bir
yaklaşım benimsemiştir.

Daha güncel olarak, derin öğrenme tabanlı yöntemler (Now Playing
[7], NEURAL-FP [8]) daha küçük model
...
- **Dinamik aralık normalizasyonu (dynaudnorm):** Mikrofonla yapılan
  ...
  literatürdeki çalışmalara [5] paralel biçimde
  artırmaktadır.
- **İkinci-en-iyi marj kontrolü:** Wang'ın orijinal algoritması en
  ...
  *Distortion Discriminant Analysis* çalışmasındaki [6]
...
Spotify'ın 2010'da yayımladığı teknik blog yazısı [18] ve
...
MediaRecorder API üzerinden gerçekleştirilmektedir [16].
...
Brookes ve arkadaşlarının [17] çalışmasında ortaya
...
gerçekleştirilmektedir [15]. Aynı dönüştürme zinciri,
...
Newman'ın *Building Microservices* [11] eserinde
...
Boot backend ve Python recognizer. Caddy [14], Let's Encrypt
...
3. Tarayıcı tarafındaki dijital sinyal işleme özelliklerinin akustik
   ...
   [17]; bu konuda yeni ölçümler katkı
   sağlamaktadır.
