...
public class FingerprintService {
    ...
    public void unregisterFingerprint(Song song) {
        if (song == null || song.getFingerprintCode() == null || song.getFingerprintCode().isBlank()) {
            return;
        }
        ...
        try {
            ...
            HttpEntity<Map<String, String>> requestEntity = new HttpEntity<>(
                    Map.of("songCode", song.getFingerprintCode()),
                    headers
            );

            restTemplate.postForEntity(
                    pythonBaseUrl + "/unregister-fingerprint",
                    requestEntity,
                    String.class
            );
        } catch (Exception e) {
